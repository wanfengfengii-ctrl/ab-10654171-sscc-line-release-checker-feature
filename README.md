...
- 批次结果采用窗口化明细：领域结果仍保存全部非空行及原始行号，页面只挂载视口与缓冲区内的行，
  三万条结果时数据行 DOM 节点始终不超过 120 个；汇总与放行结论始终基于完整批次。
- 阻断后问题游标从首个失败行开始，复核员可用“上一个问题 / 下一个问题”按输入位置跳转；
  目标行尚未挂载时先滚动再聚焦，首尾按钮禁用且不循环；空行造成的行号间隔不会让游标错位。
- 输入一旦修改，结果、窗口位置与问题游标一并清除；没有失败行时导航显示“无待处理问题”。
