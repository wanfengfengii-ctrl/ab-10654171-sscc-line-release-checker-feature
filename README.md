...
- 领域结果始终保存全部非空行（含原始行号），汇总与放行结论基于完整批次计算。
- 明细表窗口化挂载：只渲染视口与上下缓冲区附近的行（数据行 DOM 节点硬上限 120 个），
  未挂载区域用占位行撑起滚动高度；三万条结果下页面不卡顿。
- 阻断后问题游标从首个失败行开始，“上一个问题 / 下一个问题”按输入位置在失败行间跳转，
  目标行尚未挂载时先滚动视口再聚焦；首尾按钮禁用、不循环；
  空行造成的行号间隔不影响游标序号（序号按失败行的数组位置计）。
- 没有失败行时导航显示“无待处理问题”；输入一旦修改，结果、窗口位置与游标一并清除。
